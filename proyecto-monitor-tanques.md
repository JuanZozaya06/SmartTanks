...
Como compatibilidad mientras el firmware todavía no sincronice NTP, la API puede tomar `receivedAt` de la muestra más reciente como ancla y reconstruir hacia atrás usando la diferencia de `elapsedMs` cuando dispone de varios instantes comparables. Una fila aislada permanece `pending`. El resultado reconstruido siempre se marca `estimated`; no convierte la hora del servidor en una medición `verified`.
