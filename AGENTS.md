...
- Como compatibilidad para lotes sin `observedAt` que contengan más de un instante distinto, la API usa la lectura de mayor `elapsedMs` como ancla en `receivedAt`, reconstruye hacia atrás las demás lecturas del lote y guarda el resultado como `estimated`; nunca lo presenta como `verified`. Un lote con un solo instante permanece `pending`, porque podría ser una fila antigua sincronizada después. El firmware sigue siendo responsable de sincronizar NTP y enviar la hora real cuando esté disponible.
